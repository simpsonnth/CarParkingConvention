<div class="mx-auto max-w-5xl space-y-6">
        ...
        <table class="w-full min-w-[760px] text-left text-sm">
            <thead class="border-b border-zinc-200 bg-zinc-50 text-xs uppercase text-zinc-500 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-400">
                <tr>
                    ...
                    <th class="px-4 py-3 font-medium">{{ __('routes_list.col_public_access') }}</th>
                    <th class="px-4 py-3 text-end font-medium">{{ __('routes_list.open') }}</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @forelse($entries as $row)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/80" wire:key="route-row-{{ $row['route_name'] }}">
                        ...
                        <td class="px-4 py-3">
                            @if($row['toggleable'])
                                <div class="flex items-center gap-3">
                                    <flux:switch
                                        wire:click="toggleRoute('{{ $row['route_name'] }}')"
                                        :checked="$row['enabled']"
                                    />
                                    <span @class([
                                        'text-xs font-semibold uppercase tracking-wide',
                                        'text-green-600 dark:text-green-400' => $row['enabled'],
                                        'text-amber-600 dark:text-amber-400' => ! $row['enabled'],
                                    ])>
                                        {{ $row['enabled'] ? __('routes_list.status_open') : __('routes_list.status_closed') }}
                                    </span>
                                </div>
                            @else
                                <flux:badge size="sm" color="zinc">{{ __('routes_list.status_always_on') }}</flux:badge>
                            @endif
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-12 text-center text-zinc-500">{{ __('routes_list.empty') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ __('routes_list.access_note') }}</p>
</div>
